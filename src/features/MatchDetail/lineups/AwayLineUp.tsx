...
const AwayLineUp = ({ awayLineups, activeToggle, formation }) => {
  if (!awayLineups || !formation) return null;
  ...
  const groupedRows = groupedByRow(awayLineups);

  const eventTags = getEventTags(awayLineups);
        ...
        return rowPlayers.map((lineup, colIndex) => {
          ...
          return (
            <Link
              ...
              className="z-10 absolute transition duration-300 flex flex-col justify-center items-center text-white my-4"
            >
              {/* Player Image */}
